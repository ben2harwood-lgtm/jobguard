...
export function WorkspaceShell({jobId}:{jobId:string}){
 ...
 if(capture)return <main className="workspace-page" id="captured-job-workspace" data-testid="workspace-job-id" data-job-id={jobId}><PracticeJourney jobId={jobId} status={capture.status}/><Materials jobId={jobId} scopeItemId={capture.proposal.lines[0]?.scopeItemId??""}/><PurchaseOrder jobId={jobId}/><SupplierDocuments jobId={jobId}/><SupplierMatch jobId={jobId}/><ThingsToCheck jobId={jobId}/><RecoveryCases jobId={jobId}/><nav className="workspace-stages" aria-label="Saved job sections"><a href="#scope">Scope</a>{mayOpenSavedQuote(capture.status)&&<a href="#quote">Quote</a>}{hasStartedWork(capture.status)&&<><a href="#work-proof">Work and proof</a><a href="#final-account-title">Final account and invoices</a></>}</nav><ReviewProposal key={`${jobId}:${section==="scope"?"scope":"editor"}`} result={capture} initialPricing={section!=="scope"} onSnapshot={acceptSnapshot} onScope={snapshot=>{if(snapshot.jobId!==currentJobId.current)return;acceptSnapshot(snapshot);setFragment("#scope");history.replaceState(null,"",`${location.pathname}${location.search}#scope`);}} back={()=>location.assign("/")}/></main>;
 if(!job)return null;
