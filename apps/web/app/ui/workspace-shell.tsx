...
export function WorkspaceShell({ jobId }: { jobId: string }) {
  ...
  const load=useCallback(async()=>{setLoading(true);setError(null);try{const proposalResponse=await fetch(`/api/jobs/${encodeURIComponent(jobId)}/proposal`,{cache:"no-store"});if(proposalResponse.ok){const parsed=captureWorkspaceResponseV1.safeParse(await proposalResponse.json());if(!parsed.success)throw new Error("INVALID_RESPONSE");setCapture(parsed.data);return;}const response=await fetch(`/api/jobs/${encodeURIComponent(jobId)}`,{cache:"no-store"});if(!response.ok){setError(response.status===403||response.status===404?"forbidden":"unavailable");return;}const parsed=jobWorkspaceResponseV1.safeParse(await response.json());if(!parsed.success)throw new Error("INVALID_RESPONSE");setJob(parsed.data.job);}catch{setError("unavailable");}finally{setLoading(false);}},[jobId]);
  ...
  if(capture)return <main className="workspace-page"><ReviewProposal result={capture} back={()=>location.assign("/")}/></main>;
  if(!job)return null;
