...
export function browserLocalSpeechAdapter(discoverNative=true):LocalSpeechAdapter|null {
  try {
    if(typeof window==="undefined")return null;
    const injected=window.__JOBGUARD_SPEECH_ADAPTER__;
    if(injected)return injected;
    // Callers can suppress even property discovery in especially constrained browser contexts.
    if(!discoverNative)return null;
    ...
    if(typeof C!=="function" || typeof C.available!=="function" || !C.prototype || !("processLocally" in C.prototype))return null;
    return {configuration:{language:"en-GB",processLocally:true,remoteFallback:false},
      async available(){try{const value=await C.available!({langs:["en-GB"],processLocally:true});return value==="available"?"installed":value==="downloadable"?"downloadable":value==="downloading"?"downloading":"unavailable"}catch{return "unavailable"}},
      async install(){try{return typeof C.install==="function"&&(await C.install({langs:["en-GB"],processLocally:true}))===true}catch{return false}},
...
export function LocalDictation({onUse}:{onUse:(text:string,raw:string)=>void}){
  ...
  useEffect(()=>{let active=true;try{const value=browserLocalSpeechAdapter();if(!value||value.configuration.language!=="en-GB"||!value.configuration.processLocally||value.configuration.remoteFallback){setAvailability("unavailable");return}
    setAdapter(value);
    // Native capability/model discovery can take down Chromium renderers. Only deterministic injected adapters
    // are queried during mount; real browsers fail closed to the typed journey rather than doing heavy work eagerly.
    if(value!==window.__JOBGUARD_SPEECH_ADAPTER__){setAvailability("unavailable");return}
    void value.available().then(result=>{if(active)setAvailability(result)}).catch(()=>{if(active)setAvailability("unavailable")});
  }catch{setAdapter(null);setAvailability("unavailable")}
  return()=>{active=false;try{session.current?.cancel()}catch{}session.current=null}},[]);
  ...
  async function install(){if(!adapter)return;setAvailability("downloading");setMessage("Installing the on-device speech model… Progress is not provided by this browser.");try{const installed=await adapter.install(),next=installed?await adapter.available():"unavailable";if(next!=="installed"){setAvailability("unavailable");setMessage("");return}setAvailability(next);setMessage("On-device speech model installed. Choose Talk through the example when you are ready.")}catch{setAvailability("unavailable");setMessage("")}}
