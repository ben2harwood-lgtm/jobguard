...
export default async function setup() {
  ...
  const postgres = new EmbeddedPostgres({ databaseDir: directory, port, user: "postgres", password, persistent: false, createPostgresUser: process.getuid?.() === 0, initdbFlags: ["--lc-messages=C"], onLog: () => undefined });
  ...
  return async () => { await postgres.stop(); await rm(directory, { recursive: true, force: true }); };
}
