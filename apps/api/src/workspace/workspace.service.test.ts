...
import { DEMO_JOB_ID, DEMO_TENANT_ID, SyntheticDemoReadError } from "@jobguard/db";
...
describe("workspace application service", () => {
  ...
  it("maps the authoritative database projection to the versioned contract", async () => {
    read.mockResolvedValue({ job: { id: DEMO_JOB_ID, title: "Practice kitchen", status: "quoting", revision: 7, updatedAt: new Date("2026-09-15T10:00:00.000Z") } });
    await expect(new WorkspaceService({} as Pool).getJob(principal, DEMO_JOB_ID)).resolves.toEqual({ version: 1, environment: "synthetic_demo", job: { id: DEMO_JOB_ID, tenantId: DEMO_TENANT_ID, title: "Practice kitchen", status: "quoting", revision: 7, updatedAt: "2026-09-15T10:00:00.000Z" } });
  });
