...
export class CaptureService {
  ...
  async capture(context:VerifiedTenantContext,raw:unknown){const input=captureRequestV1.parse(raw);if(input.requested_tenant_id!==context.tenantId)throw new Error("TENANT_FORBIDDEN");const proposal=await extractCaptureFixture(input.captureId,input.source.text,input.fixtureId);return this.repository.persist(context,{captureId:input.captureId,text:input.source.text,proposal,promptVersion:CAPTURE_PROMPT_VERSION,schemaVersion:CAPTURE_SCHEMA_VERSION,model:PINNED_CAPTURE_MODEL});}
}
